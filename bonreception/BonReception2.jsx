...
class BonReception2 extends Component {
    state = { 
        info:{
            fournisseur:"BLABLA",
            societe:"ZALA",
     ...
     getData=async ()=>{
        ...
        if(ina.length!==0){
            let temp={
                fournisseur:ina[0].nomfournisseur,
     ...
     topdf =()=> {
        ...
        .then(response=>{
            response.blob().then(blob=>{
                ...
                alink.href = fileURL;
                alink.download = 'BDR-'+this.getdate()+this.state.info.fournisseur+'.pdf';
                alink.click();
                vita=true;
               if(vita){
    ...
    render() { 
        return (
            <div>
                <h2 className="text-center">Fournisseur {this.state.info.fournisseur}</h2>
